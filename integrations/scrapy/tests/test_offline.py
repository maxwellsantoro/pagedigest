...
class Stats:
    ...
    def inc_value(self, k, n=1): self.d[k] = self.d.get(k, 0) + n
    def get_value(self, k, default=0): return self.d.get(k, default)


def manifest_bytes(site_rev, entries, coverage=None):
    doc = {"version": 1, "generated": "2025-10-16T10:00:00Z",
           "site_rev": site_rev, "entries": entries}
    if coverage:
...
def make_mw(tmp, **over):
    s = Settings({"PAGEDIGEST_STORE": tmp, "PAGEDIGEST_MANIFEST_TTL": 0.0,
                  "PAGEDIGEST_AUDIT_RATE": 0.0, "PAGEDIGEST_BOOTSTRAP_AUDIT_RATE": 0.0})
...
def test_fallback_no_manifest():
    ...
    run(c); print("ok: fallback when no manifest")
...
def test_first_contact_then_skip():
    def c(db):
        ...
        assert r.headers.get("PageDigest-State") == b'site_rev=10; manifest="/.well-known/pagedigest.json"'
    ...
    run(c); print("ok: first contact fetches, unchanged run skips")
...
def test_changed_refetch():
    def c(db):
        set_manifest(manifest_bytes(10, {"/a": {"rev": 3}}))
        m = make_mw(db)
        r = req("/a"); m.process_request(r, None); m.process_response(r, resp(r), None)
    ...
    run(c); print("ok: bumped rev forces refetch")
...
def test_monotonicity_fallback():
    def c(db):
        ...
        assert m.process_request(req("/a"), None) is None
        assert m.stats.get_value("pagedigest/manifest_unusable") >= 1
    run(c); print("ok: site_rev decrease triggers fallback")


def test_coverage_prefixes():
    def c(db):
        raw = manifest_bytes(10, {"/blog/x": {"rev": 1}},
    ...
    run(c); print("ok: out-of-coverage URL is left alone")
...
def test_audit_detects_dishonest_digest():
    def c(db):
        ...
        m = make_mw(db, pagedigest_bootstrap_audit_rate=1.0)  # audit every skip-eligible hit
        r = req("/a"); m.process_request(r, None); m.process_response(r, resp(r, body), None)
    ...
    run(c); print("ok: digest audit catches a dishonest manifest")
...
def test_site_distrust_escalation():
    def c(db):
        ...
        m = make_mw(db, pagedigest_bootstrap_audit_rate=1.0,
                    pagedigest_site_distrust_threshold=3)
        for i in range(3):
            p = f"/p{i}"
            r = req(p); m.process_request(r, None); m.process_response(r, resp(r, b"x"), None)
            r2 = req(p); m.process_request(r2, None); m.process_response(r2, resp(r2, b"x"), None)
    ...
    run(c); print("ok: repeated mismatches escalate to site distrust + fallback")
