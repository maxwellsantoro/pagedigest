...
def main():
    ...
    print("\n==== pagedigest stats ====")
    for k in (
        "pagedigest/manifests_loaded",
        "pagedigest/skipped",
        "pagedigest/bytes_saved_est",
        "pagedigest/audits",
        "pagedigest/audit_mismatch",
        "pagedigest/manifest_unusable",
    ):
        print(f"  {k:32s} {st.get_value(k, 0)}")
    print(
        f"  {'downloader/request_count':32s} {st.get_value('downloader/request_count', 0)}"
    )
    srv.shutdown()
