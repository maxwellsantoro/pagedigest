...
- [x] Dogfood on publisher-controlled `pagedigest.org`; the checked-in site manifest is generated by the reference generator.
- [x] Document the dogfood manifest cache policy: `Cache-Control: public, max-age=300` in `site/_headers`, balancing prompt updates with inexpensive polling.
...
1. Ship generator.
2. Ship consumer.
3. Dogfood on one real site. (Completed on `pagedigest.org`.)
