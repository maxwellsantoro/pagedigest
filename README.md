...
Publisher tooling also includes `tools/reconcile_served_digests.py` (post-deploy digest convergence) and `pagedigest verify-live` (live audit sampling; `tools/verify_over_wire_digests.py` remains as a compatibility wrapper).
